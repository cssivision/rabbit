...
fn resolve01(host: &str) -> impl Future<Item = IpAddr, Error = io::Error> + Send {
...
pub async fn resolve(host: &str) -> Result<IpAddr, io::Error> {
    let v = resolve01(host).compat().await?;
    Ok(v)
}
